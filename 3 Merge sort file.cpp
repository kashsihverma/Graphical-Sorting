#include <stdlib.h>
#include<iostream>
#include<graphics.h>
using namespace std;
void Merge(int A[],int p, int q,int r)     /*It merges two arrays */
{

	int n1=q-p+1,i,j,k;       /*n1 is the size of L[]*/

	int n2=r-q;               /*n2 is the sixe of R[]*/

	int L[n1],R[n2];

	for(i=0;i<n1;i++)

	{
		L[i]=A[p+i];
	}

	for(j=0;j<n2;j++)
	{
		R[j]=A[q+j+1];
	}

	i=0,j=0;

	for(k=p;i<n1&&j<n2;k++)
	{
		if(L[i]<R[j])
		{
			A[k]=L[i++];
		}
		else
		{
			A[k]=R[j++];
		}
			for(int i1=0;i1<210;i1++)
	{
	    bar(i1*3,A[i1],(i1*3)-3,400);
	}
        delay(30);
	 cleardevice();
	}

	while(i<n1)
	{
		A[k++]=L[i++];
	}

	while(j<n2)
	{
		A[k++]=R[j++];
	}
}

void MergeSort(int A[],int p,int r)    /*It will will divide the array and sort them while merging them*/
{
	int q;

	if(p<r)
	{
		q=(p+r)/2;                      /*q is the middle element to divide the array*/
		MergeSort(A,p,q);
		MergeSort(A,q+1,r);
		Merge(A,p,q,r);
	}

}


int main()
{

	int A_Size=220;                          /*A_Size size of A[]*/

	cout<<"Total 220 bar elements :\n";
    int gd=DETECT,gm;
    initgraph(&gd,&gm,"");
    int maxix=getmaxx();
     bar(0,400,maxix,410);
     outtextxy(300,50,"Press any key to start");


	int A[A_Size];

	cout<<"Taking the array elements as random:  ";

	for(int i=0;i<A_Size;i++)
	{
		A[i]=rand()%399;
		bar(i*3,A[i],(i*3)-3,400);
	}
	getch();

	MergeSort(A,0,A_Size-1);
	for(int i1=0;i1<210;i1++)
	{
	    bar(i1*3,A[i1],(i1*3)-3,400);
	}
        delay(30);

	for(int i=0;i<A_Size;i++)
	{

		cout<<A[i]<<" ";
	}
    outtextxy(300,50,"Merge sort finish");
	getch();
    closegraph();

	cout<<endl;
}
